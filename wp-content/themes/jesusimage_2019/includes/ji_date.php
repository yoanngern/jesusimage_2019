...
function order_dates($query, $post_type = 'ji_event', $post_cat = 'ji_eventcategory', $post_status = 'publish')
{
    ...
    // only modify queries for 'ji_event' post type
 ...
 * @throws Exception
 */
function complex_month($start, $end)
{
 ...
 * @throws Exception
 */
function complex_day($start, $end)
{
    ...
    if (date_format($start, 'Y-m-d') != date_format($end, 'Y-m-d')):
        ...
        $date = date_i18n('j', $start_t) . '-' . date_i18n('j', $end_t);

    // one day
    else:

        $date = date_i18n('j', $start_t);
    ...
    if ($_service_cat === false) {
        ...
        foreach ($service_cat as $cat) {
            $special_query[] = array(
                ...
                'compare' => '=',
                'value' => get_term_by('slug', $cat, 'ji_servicecategory')->term_id,
            );
        }
    }
