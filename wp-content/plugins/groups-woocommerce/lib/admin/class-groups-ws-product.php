...
class Groups_WS_Product {
	...
	public static function init() {
		if ( is_admin() ) {
			...
			if ( defined( 'WOOOMMERCE_VERSION' ) && version_compare( 'WOOOMMERCE_VERSION', '2.6.0', '<' ) ) {
				add_action( 'woocommerce_product_write_panels',	    array( __CLASS__, 'product_data_panels' ) );
			} else {
	...
	public static function product_write_panel_tabs() {
		if ( file_exists( GROUPS_PLUGIN_DIR . '/images/groups.png' ) ) {
			echo '<style type="text/css">';
			echo '#woocommerce-product-data ul.wc-tabs li.groups_tab a:before {';
			//printf( 'content: url( %s );', GROUPS_PLUGIN_URL . 'images/groups.png' );
			printf( 'background-image: url( %s );', GROUPS_PLUGIN_URL . 'images/groups.png' );
			echo 'background-size: 12px 12px;';
			...
			echo '}';
			echo '@media only screen and (max-width:900px){';
			echo '#woocommerce-product-data ul.wc-tabs li.groups_tab a:before {';
		...
		echo '<li class="groups_tab groups_options">';
		echo '<a href="#woocommerce_groups">';
		printf( '<span title="%s">', esc_attr__( 'Groups', 'groups-woocommerce' ) );
		esc_html_e( 'Groups', 'groups-woocommerce' );
		echo '</span>';
		echo '</a>';
		echo '</li>';
	}
	...
	public static function product_data_panels() {
		...
		$groups_panel_prefix_simple .= '<p>' . __( 'The customer will be added to or removed from the selected groups when purchasing this product.', 'groups-woocommerce' ) . '</p>';
		...
		$groups_panel_prefix_subscription .= '<p>' . __( 'The customer will be a member of the selected groups as long as the subscription is active. The customer will be removed from the selected groups once the subscription is active.', 'groups-woocommerce' ) . '</p>';
		...
		$group_params = array( 'order_by' => 'name' );
		if ( apply_filters( 'groups_woocommerce_exclude_registered_group', true ) ) {
			if ( $registered_group = Groups_Group::read_by_name( Groups_Registered::REGISTERED_GROUP_NAME ) ) {
				$group_params['exclude'] = $registered_group->group_id;
			}
		}
		$groups = Groups_Group::get_groups( $group_params );

		$groups_panel_groups .= sprintf( '<div class="groups-panel-item-external " style="%s">', !$is_external ? 'display:none;' : '' );
		$groups_panel_groups .= '<p>' . __( 'Group assignments are not supported for external products.', 'groups-woocommerce' ) . '</p>';
		...
		if ( count( $groups ) == 0 ) {
			$groups_panel_groups .= '<p>' . __( 'There are no groups available to select. At least one group (other than <em>Registered</em>) must be created.', 'groups-woocommerce' ) . '</p>';
		} else {
			...
			$groups_panel_groups .= __( 'Add to Groups', 'groups-woocommerce' );
			$groups_panel_groups .= ' ';
			$groups_panel_groups .= sprintf(
				...
				esc_attr( __( 'Choose groups &hellip;', 'groups-woocommerce' ) ) ,
				esc_attr( __( 'Choose groups &hellip;', 'groups-woocommerce' ) )
			);
			...
			// remove from groups
			$groups_panel_groups .= '<label>';
			$groups_panel_groups .= __( 'Remove from Groups', 'groups-woocommerce' );
			$groups_panel_groups .= ' ';
			$groups_panel_groups .= sprintf(
		...
		switch( $duration_uom ) {
			case 'second' :
				$duration_uom_label = _n( 'Second', 'Seconds', $duration, 'groups-woocommerce' );
				break;
			case 'minute' :
				$duration_uom_label = _n( 'Minute', 'Minutes', $duration, 'groups-woocommerce' );
				break;
			case 'hour' :
				$duration_uom_label = _n( 'Hour', 'Hours', $duration, 'groups-woocommerce' );
				break;
			case 'day' :
				$duration_uom_label = _n( 'Day', 'Days', $duration, 'groups-woocommerce' );
				break;
			case 'week' :
				$duration_uom_label = _n( 'Week', 'Weeks', $duration, 'groups-woocommerce' );
				break;
			case 'year' :
				$duration_uom_label = _n( 'Year', 'Years', $duration, 'groups-woocommerce' );
				break;
			default :
				$duration_uom_label = _n( 'Month', 'Months', $duration, 'groups-woocommerce' );
				break;
		}

		$duration_help =
			__( 'Leave the duration empty unless you want memberships to end after a certain amount of time.', 'groups-woocommerce' ) .
			' ' .
			__( 'If the duration is empty, the customer will remain a member of the selected groups forever, unless removed explicitly.', 'groups-woocommerce' ) .
			' ' .
			__( 'If the duration is set, the customer will only belong to the selected groups during the specified time, based on the <em>Duration</em> and the <em>Time unit</em>.', 'groups-woocommerce' );
		...
		woocommerce_wp_text_input(
			array(
				'id'          => '_groups_duration',
				'label'       => sprintf( __( 'Duration', 'groups-woocommerce' ), $duration_help_icon ),
				'value'       => $duration,
				'description' => sprintf( __( '%s (as chosen under <em>Time unit</em>)', 'groups-woocommerce' ), $duration_uom_label ),
				'placeholder' => __( 'unlimited', 'groups-woocommerce' )
			)
		);
		...
		woocommerce_wp_select(
			array(
				...
				'label'       => __( 'Time unit', 'groups-woocommerce' ),
				'value'       => $duration_uom,
				'options'     => array(
					'second' => __( 'Seconds', 'groups-woocommerce' ),
					'minute' => __( 'Minutes', 'groups-woocommerce' ),
					'hour'   => __( 'Hours', 'groups-woocommerce' ),
					'day'    => __( 'Days', 'groups-woocommerce' ),
					'week'   => __( 'Weeks', 'groups-woocommerce' ),
					'month'  => __( 'Months', 'groups-woocommerce' ),
					'year'   => __( 'Years', 'groups-woocommerce' ),
				)
			)
		);
		$groups_panel_suffix_simple .= ob_get_clean();
		...
		echo '<p>' . __( 'Note that all users belong to the <em>Registered</em> group automatically.', 'groups-woocommerce' ) . '</p>';
		echo '<br/>';
	 ...
	 * @param WP_Post $variation (this really is an object of type WP_Post and not a WC_Product, so the WC 3.0.0 getters don't apply here (last checked at WC 3.1.0-alpha)
	 */
	public static function woocommerce_product_after_variable_attributes( $loop, $variation_data, $variation ) {
		...
		// add to groups
		$output .= '<label>';
		$output .= __( 'Add to Groups', 'groups-woocommerce' );
		$output .= ' ';
		$output .= sprintf(
		...
		$output .= '<p class="description">' . __( 'Add the customer to these groups when purchasing this variation.', 'groups-woocommerce' ) . '</p>';

		// remove from groups
		$output .= '<label>';
		$output .= __( 'Remove from Groups', 'groups-woocommerce' );
		$output .= ' ';
		$output .= sprintf(
		...
		$output .= '<p class="description">' . __( 'Remove the customer from these groups when purchasing this variation.', 'groups-woocommerce' ) . '</p>';
		...
		$output .= '<p>' . __( 'If set, the duration limitations in the <em>Groups</em> settings of the variable product apply.', 'groups-woocommerce' ) . '</p>';
		$output .= '</div>';
	...
	public static function woocommerce_get_price_html( $price, $product ) {
		...
		if ( $show_duration ) {
			$duration     = get_post_meta( $product->get_id(), '_groups_duration', true );
			if ( !empty( $duration ) ) {
				$duration_uom = get_post_meta( $product->get_id(), '_groups_duration_uom', true );
				switch( $duration_uom ) {
					case 'second' :
						$price = sprintf( _n( '%s for 1 second', '%s for %d seconds', $duration, 'groups-woocommerce' ), $price, $duration );
						break;
					case 'minute' :
						$price = sprintf( _n( '%s for 1 minute', '%s for %d minutes', $duration, 'groups-woocommerce' ), $price, $duration );
						break;
					case 'hour' :
						$price = sprintf( _n( '%s for 1 hour', '%s for %d hours', $duration, 'groups-woocommerce' ), $price, $duration );
						break;
					case 'day' :
						$price = sprintf( _n( '%s for 1 day', '%s for %d days', $duration, 'groups-woocommerce' ), $price, $duration );
						break;
					case 'week' :
						$price = sprintf( _n( '%s for 1 week', '%s for %d weeks', $duration, 'groups-woocommerce' ), $price, $duration );
						break;
					case 'year' :
						$price = sprintf( _n( '%s for 1 year', '%s for %d years', $duration, 'groups-woocommerce' ), $price, $duration );
						break;
					default :
						$price = sprintf( _n( '%s for 1 month', '%s for %d months', $duration, 'groups-woocommerce' ), $price, $duration );
						break;
				}
			}
		}
	...
	public static function has_duration( $product ) {
		...
		if ( $product->get_type() == 'variation' ) {
			$duration = get_post_meta( $product->get_parent_id(), '_groups_duration', true );
		}
		return $duration > 0;
	}
	 ...
	 * @param WC_Product $product
	 * @return int duration in seconds or null if there is none defined
	 */
	public static function get_duration( $product ) {
		...
		if ( !empty( $duration ) ) {
			$duration_uom = get_post_meta( $product->get_id(), '_groups_duration_uom', true );
			if ( $product->get_type() == 'variation' ) {
				$duration_uom = get_post_meta( $product->get_parent_id(), '_groups_duration_uom', true );
			}
			$suffix = $duration > 1 ? 's' : '';
	 ...
	 * @return int seconds or null if $duration is empty
	 */
