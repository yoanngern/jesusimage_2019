...
class Groups_WS_Handler {
	...
	public static function woocommerce_subscription_status_updated( $subscription, $new_status, $old_status ) {
		...
		if ( !apply_filters( 'groups_woocommerce_handle_woocommerce_subscription_status_updated', true, $subscription, $new_status, $old_status ) ) {
			if ( GROUPS_WS_LOG ) {
				error_log( sprintf( __METHOD__ . ' not acting on update for subscription status %s', esc_html( $new_status ) ) );
			}
			return;
		}
	 ...
	 * 
	 * @param WC_Subscription $subscription
	 * @param WC_Order_Item_Product $new_order_item note that this is an object (unlike the third parameter)
	 * @param array $switched_order_item note that this is an array holding data for the switched order item
	 */
